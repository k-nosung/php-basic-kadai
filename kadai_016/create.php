...
// MAMPを利用しているMacユーザーの方は、''ではなく'root'を代入してください
 ...
 if (isset($_GET['id'])) {
     try {
         ...
         // genesテーブルからjenre__codeカラムのデータを取得するためのSQL文を変数$sql_select_vendor_codesに代入する
         $sql_select_genes_codes = 'SELECT jenre_code FROM genes';
 
         // SQL文を実行する
         $stmt_select_jenre_codes = $pdo->query($sql_select_jenre__codes);
 
         // SQL文の実行結果を配列で取得する
         // 補足：PDO::FETCH_COLUMNは1つのカラムの値を1次元配列（多次元ではない普通の配列）で取得する設定である
         $jenre__codes = $stmt_select_jenre__codes->fetchAll(PDO::FETCH_COLUMN);
     } catch (PDOException $e) {
         exit($e->getMessage());
     }
 } else {
 ...
 <body>
     ...
     <footer>
         <p class="copyright">&copy; 書籍
            管理アプリ All rights reserved.</p>
     </footer>
 </body>
